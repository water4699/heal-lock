...
export const createFhevmInstance = async (parameters: {
  ...
  if (isMock && rpcUrl) {
    ...
    if (fhevmRelayerMetadata) {
      ...
      notify("creating");
      try {
        // Dynamically import mock to avoid including it in production bundle
        ...
        if (error.message?.includes("Cannot find module") || error.message?.includes("Failed to fetch")) {
          console.warn("[FHEVM] Mock FHEVM not available, falling back to relayer SDK");
        } else {
          throw error;
        }
      }
    } else {
      console.warn("[FHEVM] Hardhat node FHEVM metadata not found. Using mock FHEVM instance.");
    }
  }
